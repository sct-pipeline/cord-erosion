...
# Tested with SCT cd64e67844e7e4db62665d0e076a1dd816298ac2
...
# Run erosion on the PAM50 template
mkdir erosion
cd erosion
# Initialize PAM50_cord_rings.nii.gz file from cord segmentation
cp $SCT_DIR/data/PAM50/template/PAM50_cord.nii.gz PAM50_cord_rings.nii.gz
# Erode PAM50 cord mask several times, until there is nothing left
for size in 1 2 3 4 5 6 7 8; do
  sct_maths -i $SCT_DIR/data/PAM50/template/PAM50_cord.nii.gz -erode $size -shape disk -dim 2 -o PAM50_cord_e${size}.nii.gz
	# Iteratively add mask in order to create rings: each ring will have a specific label value: 1 (most outer ring), 2, 3, 4, etc.
	sct_maths -i PAM50_cord_rings.nii.gz -add PAM50_cord_e${size}.nii.gz -o PAM50_cord_rings.nii.gz
done

# Iterate across subjects and quantify in rings
...
# 	# sct_maths -i lesion_merged.nii.gz -erode
